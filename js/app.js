...
if (currentMapId && typeof cityMaps !== 'undefined' && cityMaps[currentMapId]) {
    ...
    mapSvg.setAttribute('data', cityMapData.svgFile);
    activeCityIds = cityMapData.cityIds;
    activeCities = cityMapData.cities;
    isSubMap = true;
    document.title = cityMapData.displayName + ' - Curse of Strahd';
}

// Mostrar botão de voltar quando em sub-mapa
if (isSubMap) {
    const backBtn = document.getElementById('back-to-barovia');
    if (backBtn) {
...
function initSvg() {
    ...
    style.textContent = `
        ...
        /* Blood drawing animation */
        @keyframes blood-draw {
            0% { stroke-dashoffset: var(--path-length); stroke: rgba(100,5,5,0); }
            5% { stroke: rgba(100,5,5,0.95); }
            100% { stroke-dashoffset: 0; stroke: rgba(100,5,5,0.95); }
        }
        @keyframes blood-pulse {
            0%, 100% {
                stroke-width: 7px;
                filter: drop-shadow(0 0 3px rgba(80,0,0,0.6));
            }
            50% {
                stroke-width: 8.5px;
                filter: drop-shadow(0 0 6px rgba(120,5,5,0.8));
            }
        }
        @keyframes drip-grow {
            0% { stroke-dashoffset: var(--drip-len); opacity: 0; }
            10% { opacity: 0.9; }
        ...
        .blood-path.blood-active {
            stroke-dasharray: var(--path-length);
            stroke-dashoffset: 0;
            stroke: rgba(100,5,5,0.95) !important;
            stroke-width: 7px !important;
            stroke-linecap: round;
            animation: blood-draw 1.2s ease-out forwards, blood-pulse 2.8s ease-in-out 1.2s infinite;
        }
        .blood-drips .drip-line {
            stroke-dasharray: var(--drip-len);
            stroke-dashoffset: var(--drip-len);
            opacity: 0;
        }
    `;
    svgDoc.querySelector('svg').appendChild(style);

    // Criar overlay escuro (usando path com recorte dinâmico)
    // Inserir ANTES do grupo de POIs para que os POIs fiquem renderizados acima
    const svgEl = svgDoc.querySelector('svg');
    const poisGroup = svgDoc.getElementById('points-of-interest');
    const overlay = svgDoc.createElementNS('http://www.w3.org/2000/svg', 'path');
    ...
    if (poisGroup) {
        svgEl.insertBefore(overlay, poisGroup);
    } else {
    ...
    // Middle-click pan dentro do SVG
    let svgDragging = false;
    let svgStartX, svgStartY, svgLastTX, svgLastTY;
    svgDoc.addEventListener('mousedown', (e) => {
        if (e.button === 1) {
            ...
            svgStartX = e.screenX;
            svgStartY = e.screenY;
            svgLastTX = translateX;
            svgLastTY = translateY;
    ...
    svgDoc.addEventListener('mousemove', (e) => {
        ...
        e.preventDefault();
        translateX = svgLastTX + (e.screenX - svgStartX);
        translateY = svgLastTY + (e.screenY - svgStartY);
        updateTransform();
    });
    ...
    // ===== ANIMAÇÃO DE SANGUE - TODOS OS POIs PATH =====
    function createBloodEffects(svgDoc, pathEl, groupEl) {
        const bbox = pathEl.getBBox();
        const totalLen = pathEl.getTotalLength();
        const bloodGroup = svgDoc.createElementNS('http://www.w3.org/2000/svg', 'g');
        bloodGroup.classList.add('blood-drips');
        bloodGroup.style.opacity = '0';

        // --- DRIPS: muitas gotas escorrendo do contorno ---
        const dripCount = Math.min(60, Math.max(35, Math.floor(totalLen / 20)));
        for (let i = 0; i < dripCount; i++) {
            const t = ((i + Math.random() * 0.7) / dripCount) % 1;
            const point = pathEl.getPointAtLength(t * totalLen);

            // Tamanhos muito variados
            const sizeRoll = Math.random();
            let dripLen;
            if (sizeRoll < 0.2) dripLen = 5 + Math.random() * 6;        // tiny: 5-11px
            else if (sizeRoll < 0.5) dripLen = 11 + Math.random() * 15;  // short: 11-26px
            else if (sizeRoll < 0.8) dripLen = 26 + Math.random() * 30;  // medium: 26-56px
            else dripLen = 56 + Math.random() * 40;                       // long: 56-96px

            const thickness = 2 + Math.random() * 4.5; // 2-6.5px
            const endY = point.y + dripLen;
            const wobbleX = point.x + (Math.random() - 0.5) * 4;

            const dripPath = svgDoc.createElementNS('http://www.w3.org/2000/svg', 'path');
            const midY = point.y + dripLen * (0.35 + Math.random() * 0.3);
            dripPath.setAttribute('d', `M${point.x},${point.y} C${point.x},${midY} ${wobbleX},${midY} ${wobbleX},${endY}`);
            dripPath.setAttribute('class', 'drip-line');

            const r = 80 + Math.floor(Math.random() * 35);
            const g = Math.floor(Math.random() * 8);
            const b = Math.floor(Math.random() * 8);
            dripPath.style.fill = 'none';
            dripPath.style.stroke = `rgb(${r},${g},${b})`;
            dripPath.style.strokeWidth = thickness + 'px';
            dripPath.style.strokeLinecap = 'round';
            dripPath.style.setProperty('--drip-len', dripLen.toFixed(0));

            // Timing rápido e escalonado
            const delay = 0.8 + i * 0.12 + Math.random() * 0.3;
            const duration = 0.8 + Math.random() * 1.2;
            dripPath.dataset.delay = delay.toFixed(2);
            dripPath.dataset.duration = duration.toFixed(2);

            bloodGroup.appendChild(dripPath);
        }

        // --- MANCHAS DE SANGUE INTERNAS ---
        const splatterCount = Math.min(18, Math.max(8, Math.floor(totalLen / 60)));
        for (let i = 0; i < splatterCount; i++) {
            // Pontos aleatórios DENTRO do bounding box do path
            const cx = bbox.x + bbox.width * (0.15 + Math.random() * 0.7);
            const cy = bbox.y + bbox.height * (0.15 + Math.random() * 0.7);

            const splatter = svgDoc.createElementNS('http://www.w3.org/2000/svg', 'g');
            splatter.setAttribute('class', 'blood-splatter');
            splatter.style.opacity = '0';
            splatter.style.transformOrigin = `${cx}px ${cy}px`;

            // Forma irregular da mancha (blob com múltiplos pontos)
            const numPoints = 5 + Math.floor(Math.random() * 4);
            const baseRadius = 4 + Math.random() * 12;
            let blobD = '';
            for (let p = 0; p <= numPoints; p++) {
                ...
                if (p === 0) blobD += `M${px.toFixed(1)},${py.toFixed(1)} `;
                else {
                    const cpAngle = ((p - 0.5) / numPoints) * Math.PI * 2;
                    const cpR = baseRadius * (0.8 + Math.random() * 0.6);
                    const cpx = cx + Math.cos(cpAngle) * cpR;
                    const cpy = cy + Math.sin(cpAngle) * cpR;
                    blobD += `Q${cpx.toFixed(1)},${cpy.toFixed(1)} ${px.toFixed(1)},${py.toFixed(1)} `;
                }
            }
            blobD += 'Z';

            const blobPath = svgDoc.createElementNS('http://www.w3.org/2000/svg', 'path');
            blobPath.setAttribute('d', blobD);
            const r = 75 + Math.floor(Math.random() * 35);
            blobPath.style.fill = `rgba(${r},3,3,0.55)`;
            blobPath.style.stroke = 'none';

            splatter.appendChild(blobPath);

            // Pequenos salpicos ao redor da mancha
            const splatCount = 2 + Math.floor(Math.random() * 3);
            for (let s = 0; s < splatCount; s++) {
                const sAngle = Math.random() * Math.PI * 2;
                const sDist = baseRadius + 2 + Math.random() * 8;
                const sx = cx + Math.cos(sAngle) * sDist;
                const sy = cy + Math.sin(sAngle) * sDist;
                const sR = 1.5 + Math.random() * 3;
                const splatCircle = svgDoc.createElementNS('http://www.w3.org/2000/svg', 'circle');
                splatCircle.setAttribute('cx', sx.toFixed(1));
                splatCircle.setAttribute('cy', sy.toFixed(1));
                splatCircle.setAttribute('r', sR.toFixed(1));
                splatCircle.style.fill = `rgba(${r},2,2,0.5)`;
                splatter.appendChild(splatCircle);
            }

            // Timing da mancha
            const delay = 1.0 + i * 0.3 + Math.random() * 0.5;
            const duration = 0.6 + Math.random() * 0.5;
            splatter.dataset.delay = delay.toFixed(2);
            splatter.dataset.duration = duration.toFixed(2);

            bloodGroup.appendChild(splatter);
        }

        groupEl.appendChild(bloodGroup);
        return bloodGroup;
    }
    ...
    activeCityIds.forEach(id => {
        ...
        pathEl.classList.add('blood-path');
        ...
        // Click handler - blood animation toggle
        group.addEventListener('click', (e) => {
            e.stopPropagation();
            if (pathEl.classList.contains('blood-active')) {
                // Deactivate
                pathEl.classList.remove('blood-active');
                pathEl.style.strokeDashoffset = pathLength;
                pathEl.style.stroke = 'rgba(100,5,5,0)';
                pathEl.style.animation = 'none';
                bloodGroup.style.opacity = '0';
                bloodGroup.querySelectorAll('.drip-line').forEach(line => {
                    line.style.animation = 'none';
                });
                bloodGroup.querySelectorAll('.blood-splatter').forEach(sp => {
                    sp.style.opacity = '0';
                    sp.style.animation = 'none';
                });
            } else {
                // Activate blood drawing
                pathEl.style.animation = 'none';
                pathEl.offsetHeight;
                pathEl.style.animation = '';
                pathEl.classList.add('blood-active');

                // Activate drips
                bloodGroup.style.opacity = '1';
                bloodGroup.querySelectorAll('.drip-line').forEach(line => {
                    line.style.animation = 'none';
                    line.offsetHeight;
                    const delay = line.dataset.delay;
                    const duration = line.dataset.duration;
                    line.style.animation = `drip-grow ${duration}s ease-in ${delay}s forwards`;
                });
                // Activate splatters
                bloodGroup.querySelectorAll('.blood-splatter').forEach(sp => {
                    sp.style.animation = 'none';
                    sp.offsetHeight;
                    const delay = sp.dataset.delay;
                    const duration = sp.dataset.duration;
                    sp.style.animation = `splatter-appear ${duration}s ease-out ${delay}s forwards`;
                });
            }
            // If city has a map file, open city map modal instead of wiki
            if (activeCities[id] && activeCities[id].mapFile) {
...
function selectCity(id) {
    ...
    activeCityIds.forEach(cid => {
        ...
        if (g) {
            ...
            g.style.filter = '';
            // Reset blood on other cities when selecting new one
            if (cid !== id) {
                const bPath = g.querySelector('.blood-path');
                if (bPath && bPath.classList.contains('blood-active')) {
                    const pl = bPath.getTotalLength();
                    bPath.classList.remove('blood-active');
                    bPath.style.strokeDashoffset = pl;
                    bPath.style.stroke = 'rgba(100,5,5,0)';
                    bPath.style.animation = 'none';
                }
                const drips = g.querySelector('.blood-drips');
                if (drips) {
                    drips.style.opacity = '0';
                    drips.querySelectorAll('.drip-line').forEach(l => { l.style.animation = 'none'; });
                }
            }
        }
    });
    ...
    if (group) {
        ...
        if (poiPath) {
            ...
            let tx = 0, ty = 0;
            let el = poiPath.parentElement;
            while (el && el.tagName !== 'svg') {
                const t = el.getAttribute('transform');
                if (t) {
                    // Parse matrix(a,b,c,d,e,f) - pegar e,f (translação)
                    ...
                    if (translateMatch) {
                        const vals = translateMatch[1].split(',').map(Number);
                        tx += vals[0] || 0;
                        ty += vals[1] || 0;
                    }
                }
            ...
            if (tx !== 0 || ty !== 0) {
                // Path tem transform - reconstruir pontos traduzidos
                const totalLen = poiPath.getTotalLength();
                const steps = Math.max(80, Math.floor(totalLen / 8));
                let cutoutD = '';
                for (let i = 0; i <= steps; i++) {
                    const pt = poiPath.getPointAtLength((i / steps) * totalLen);
                    const x = pt.x + tx;
                    const y = pt.y + ty;
    ...
    if (svgDoc) {
        ...
        activeCityIds.forEach(id => {
            ...
            if (g) {
                g.classList.remove('active', 'city-highlighted');
                g.style.filter = '';
                // Reset blood animation
                const pathEl = g.querySelector('.blood-path');
                if (pathEl && pathEl.classList.contains('blood-active')) {
                    const pathLength = pathEl.getTotalLength();
                    pathEl.classList.remove('blood-active');
                    pathEl.style.strokeDashoffset = pathLength;
                    pathEl.style.stroke = 'rgba(100,5,5,0)';
                ...
                if (drips) {
                    drips.style.opacity = '0';
                    drips.querySelectorAll('.drip-line').forEach(line => {
                        line.style.animation = 'none';
                    });
                }
            }
...
// ===== PAN (ARRASTAR MAPA COM MIDDLE CLICK) =====
...
document.addEventListener('mousemove', (e) => {
    ...
    translateX = lastTranslateX + (e.screenX - startX);
    translateY = lastTranslateY + (e.screenY - startY);
    updateTransform();
});
...
function buildEntityMap() {
    ...
    // Locais
    if (typeof locationNameToSvgId !== 'undefined') {
...
if (sessionModalOverlay) sessionModalOverlay.addEventListener('click', (e) => {
...
(function() {
    ...
    if (trailEditBtn) trailEditBtn.addEventListener('click', () => {
        ...
        journeyDeleteBtn.style.display = 'block';
